...
def calc_features(l_p, l_z):
    ...
    feature_1 = desired_dim[1] - l_t_p[0][1]  # altura de cabeza con punto máximo: resolución_y - cabeza_y
    feature_2 = abs(desired_dim[0] - l_t_p[1][0])  # punto distal del punto medio (izquierda o derecha): |resolución_x/2 - pecho_x|
    feature_3 = l_t_p[1][2]  # profundidad desde un punto medial: pecho_y

    client.send_message("/height", feature_1)
    client.send_message("/medio", feature_2)
    client.send_message("/depth", feature_3)

    df = pd.DataFrame({'Height': feature_1, 'MDistance': feature_2, 'Depth': feature_3})
...
def obtain_depth(l_p):
    ...
    def mean_rectangle_dist(p, r=t_radius):
